function [ output ] = CHFM_D(img,maxorder)
[N, M]  = size(img);
x       = -1+1/M:2/M:1-1/M;
y       = 1-1/N:-2/N:-1+1/N;
[X,Y]   = meshgrid(x,y);
...
for order=0:1:maxorder
    for repetition=-maxorder:1:maxorder
        output(order+1,repetition+maxorder+1)=getChebyshevFourierMoments(img,rho,theta,order,repetition);
    end
end
end

function [output] = getChebyshevFourierMoments(img,rho,theta,order,repetition)
R=getRadialPoly(order,rho);       % get the radial polynomial
pupil =R.*exp(-1j*repetition * theta);
Product = double(img) .* pupil;
cnt = nnz(R)+1;
output = sum(Product(:))*(4/cnt)*(4/pi^2);       % calculate the moments
end
