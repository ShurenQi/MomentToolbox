%% Direct
% function [ output ] = GPCET_D(img,maxorder,alpha)
% [N, M]  = size(img);
% x       = -1+1/M:2/M:1-1/M;
% y       = 1-1/N:-2/N:-1+1/N;
% [X,Y]   = meshgrid(x,y);
...
%     R=sqrt(alpha*rho.^(alpha-2)/(2*pi)).*exp(-1j*2*pi*order.*(rho.^alpha));
%     for repetition=-maxorder:1:maxorder
%         pupil =R.*exp(-1j*repetition * theta);
%         Product = double(img) .* pupil;
%         cnt = nnz(R)+1;
%         output(order+maxorder+1,repetition+maxorder+1)= sum(Product(:))*(4/cnt);       % calculate the moments
%     end
% end
% end
