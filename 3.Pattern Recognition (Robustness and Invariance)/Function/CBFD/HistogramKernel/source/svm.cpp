 ...
 gamma(param.gamma), coef0(param.coef0)
{
	switch(kernel_type)
	{
		...
		case SUMMIN:
			...
			break;
		case CHISQUARE:
			kernel_function = &Kernel::kernel_chi_square;
			break;
		case PRECOMPUTED:
...
double Kernel::chi_square( const svm_node *px, const svm_node *py )
{
	...
	while ( px->index != -1 && py->index != -1 ) {
		if ( px->index == py->index ) {
			diff_square = (px->value - py->value);

			if(px->value + py->value > 0)
				sum += diff_square / (px->value + py->value) * diff_square;
				++px;
				++py;
		}
		else {
			//If px py, then either px->value or py->value == 0, in such case the distance is px->value or py->value and should be added
			if ( px->index > py->index ) {
				sum += py->value;
				++py;
			}
			  ...
			  const svm_parameter& param)
{
	switch(param.kernel_type)
	{
		...
		case SUMMIN:
			return sum_min( x, y );
		case CHISQUARE:
			return exp( -param.gamma * chi_square(x,y) );
